...
// Interface for Updating an RBM (Code replica, because NULL parameters derp)
SEXP _C_RBM_update(SEXP dataset, SEXP batch_size, SEXP n_hidden, SEXP training_epochs,
           SEXP learning_rate, SEXP momentum, SEXP rand_seed,
           SEXP init_W, SEXP init_hbias, SEXP init_vbias)
{
 	int nrow = INTEGER(GET_DIM(dataset))[0];
 	int ncol = INTEGER(GET_DIM(dataset))[1];
	...
	// Perform Training
	RBM rbm;
	train_rbm (&rbm, train_X_p, nrow, ncol, basi, nhid, trep, lera, mome, rase, initial_W, initial_hbias, initial_vbias);
