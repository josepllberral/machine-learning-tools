...
// This function reads a dataset into a matrix. Consider a matrix of Images x Features x Sequences
//   param dataset     : empty matrix to fill with sequential data
//   param filename_bd : filename for the dataset to read
//   param nrow        : number of images in the dataset
//   param ncol        : number of features for each image
//   param seq_len     : empty vector to fill with indexes for sequences in data
//   param filename_sl : filename for the sequence indices to read
//   param nseq        : number of sequences in data
void read_dataset (gsl_matrix* dataset, char* filename_bd, int nrow, int ncol,
					int* seq_len, char* filename_sl, int nseq)
{
	FILE * fp;
	char * line = NULL;
	size_t len = 0;
...
/*---------------------------------------------------------------------------*/
/* MAIN FUNCTION - RECONSTRUCTION EXAMPLE                                    */
/*---------------------------------------------------------------------------*/
...
int main (int argc, char** argv)
{
	if (argc != 13)
	{
		printf("Usage: crbm <batch size> <hidden units> <delay> <epochs> <learning rate> <momentum> <random seed> <training file> <num images> <num features> <sequence index file> <num sequences>\n");
		printf("Example: rbm 100 100 6 300 1e-4 0.8 1234 motion_bd.data 3826 49 motion_sl.data 3\n");
		exit(0);
	}

	int basi = atoi(argv[1]);		// Batch Size, e.g. 100
	int nhid = atoi(argv[2]);		// Num. Hidden Units, e.g. 100
	int dely = atoi(argv[3]);		// Delay, e.g. 6
	int trep = atoi(argv[4]);		// Training Epochs, e.g. 300
	double lera = atof(argv[5]);	// Learning Rate, e.g. 1e-4
	double mome = atof(argv[6]);	// Momentum, e.g. 0.8
	int rase = atoi(argv[7]);		// Random Seed, e.g. 1234

	char* filename_bd = argv[8];	// Training Filename with Data
	int nrow = atoi(argv[9]);		// Number Images in Dataset, e.g. 3826
	int ncol = atoi(argv[10]);		// Number Features per Image, e.g. 49

	char* filename_sl = argv[11];	// Training Filename with Sequence Indexes
	int nseq = atoi(argv[12]);		// Number of Sequences, e.g. 3

	// Read Batchdata
	gsl_matrix* train_X_p = gsl_matrix_alloc(nrow, ncol);
	int* seq_len_p = malloc(sizeof(int) * nseq);
	read_dataset(train_X_p, filename_bd, nrow, ncol, seq_len_p, filename_sl, nseq);

	printf("Read Batch Data & Sequences Length\n");
	...
	// Check Error again
	double error_recons = check_error(train_X_p, reconstruction, nrow, ncol);
