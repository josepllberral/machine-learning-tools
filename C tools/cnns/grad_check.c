/*---------------------------------------------------------------------------*/
/* GRADIENT CHECK for CONVOLUTIONAL NEURAL NETWORKS in C                     */
/*---------------------------------------------------------------------------*/
...
int check_grad_flat (FLAT* flat, gsl_matrix*** x0, int rand_seed, double eps, double rtol, double atol)
{
	...
	if (is_equal > 0)
	{
		...
		for (int b = 0; b < flat->batch_size; b++)
			for (int c = 0; c < flat->n_channels; c++)
				for (int i = 0; i < x0[b][c]->size1; i++)
					for (int j = 0; j < x0[b][c]->size2; j++)
						printf("%d,%d,%d,%d: %f, %f\n", b, c, i, j, gsl_matrix_get(x0[b][c], i, j), gsl_matrix_get(dx[b][c], i, j));
